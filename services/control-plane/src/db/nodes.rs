...
pub async fn insert_node_tx(
    ...
    sqlx::query(
        "INSERT INTO nodes (
            ...
            disk_available_bytes, health_message, workload_count, created_at, updated_at
        ) VALUES (?, ?, ?, 'enrolling', ?, ?, ?, ?, ?, ?, ?, ?, NULL, 0, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?)",
    )
...
pub async fn list(pool: &MySqlPool) -> Result<Vec<NodeRecord>, sqlx::Error> {
    let rows = sqlx::query_as::<_, NodeRow>(
        "SELECT id, name, hostname, status, agent_version, protocol_version, architecture, operating_system,
